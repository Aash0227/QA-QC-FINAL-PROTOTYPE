...
- **Not a Windows service** (NSSM / `sc create` run in session 0, which cannot
  reach the interactive Revit UI the bridge drives — Revit-live features break
  silently while the HTTP API keeps answering). Auto-start via a Startup-folder
  shortcut (`shell:startup`) to `run_backend.ps1 -Prod`, Run: Minimized, on the
  same machine as Revit; Task Scheduler "At log on of \<user\>" + "Run only when
  user is logged on" is the upgrade path when restart-on-crash matters.
  uvicorn workers=1 (artifact store + bridge lock are single-process by design).
