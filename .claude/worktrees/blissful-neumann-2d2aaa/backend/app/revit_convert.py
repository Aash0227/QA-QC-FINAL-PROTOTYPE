...
def _learned_key_points(
    ...
    return {
        ...
        "naming_patterns_observed": [
            "Body families carry the structural hold-down token (e.g. SHDU11, SHDU6, "
            "SHD10S, SHD15B), often with a '-With Bolt' / '-2 Sided' suffix.",
            "Evidence-member families embed keywords such as 'Anchor_Bolt_*' and "
            "'*offset' and are co-located with a body record.",
            "scheduled_type provides a clean core token (HDU6/HDU11/HD10S/HD15B) and is "
            "preferred over the raw family string for classification.",
        ],
...
def _llm_enrichment(family_dict: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    system_prompt = (
        ...
        "(HDU6, HDU11, HD10S, HD15B) when identifiable. Reply ONLY with JSON of shape "
...
def convert_revit(raw_json: dict[str, Any], source_file: str) -> dict[str, Any]:
    ...
    for mark in ("H1", "H2", "H3", "H4"):
        rev = body_by_mark.get(mark, 0)
        if rev != pdf_baseline[mark]:
            qa_warnings.append(
                f"{mark}: Revit body assemblies={rev} vs PDF baseline={pdf_baseline[mark]} "
                f"(delta {rev - pdf_baseline[mark]}). Do NOT force-match; flag for review."
            )
    if orphan_members:
        qa_warnings.append(
    ...
    if all_views_null:
        scope_warnings.append(
            "All records have view=null; cannot scope to the S-201 comparison view."
        )
    revit_count_diagnostics = {
        "raw_holdown_records": len(records),
        "canonical_assemblies": len(assemblies),
        "by_mark": {m: body_by_mark.get(m, 0) for m in ("H1", "H2", "H3", "H4")},
        "pdf_baseline": dict(pdf_baseline),
