...
- Run as a Windows service with **nssm** or Task Scheduler on the same machine as Revit (the MCP bridge requires the interactive Revit session). Exactly **one** uvicorn worker — the artifact store and bridge lock are single-process by design.
