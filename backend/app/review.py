...
def load() -> dict[str, Any]:
    ...
    if p.exists():
        try:
            ...
            import logging
            logging.warning("review.load: corrupt JSON at %s — resetting: %s", p, exc)
...
def add_comment(element: dict[str, Any], comment: str, author: str | None) -> dict[str, Any]:
    ...
    data = load()
    # max+1, not len+1: after a delete, len+1 re-issues an existing id
    next_num = 1 + max(
        (int(c["id"].rsplit("_", 1)[-1]) for c in data["comments"]
         if str(c.get("id", "")).rsplit("_", 1)[-1].isdigit()),
        default=0,
    )
    entry = {
        "id": f"rc_{next_num:03d}",
...
def analyze(element_id: str, element_list: dict[str, Any],
    ...
    dist = device.get("distance_ft") if device.get("distance_ft") is not None else math.hypot(vx, vy)
    ...
    peers = [d for d in registry.get("categories", {}).get(cat, {}).get("devices", [])
