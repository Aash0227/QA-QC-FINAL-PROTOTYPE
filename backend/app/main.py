...
def create_app() -> FastAPI:
    # Deferred config init (was module-level side effects).
    config._init()
    ...
    async def _require_token(request, call_next):
        if AUTH_TOKEN and request.url.path.startswith("/api/") \
            ...
            # SECURITY NOTE (?token= query-param auth):
            #   The query-param token is the SAME bearer token. It leaks via
            #   browser history, proxy logs, and Referer headers. EventSource
            #   (SSE), <img> evidence crops, and window.open exports cannot set
            #   an Authorization header, so we accept ?token= as a fallback.
            #   The token value is redacted from logs by RedactSecretsFilter
            #   in maintenance.py. A future improvement would use a short-lived
            #   one-time token for query-param auth, separate from the bearer
            #   token. For now, the same token is accepted from both sources.
