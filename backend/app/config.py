...
def _migrate_legacy_layout() -> None:
    """One-time: move flat artifacts/* into artifacts/projects/<slug>/.

    Uses the active project slug from ACTIVE_PROJECT_FILE when available.
    If no active project file exists, skip the migration gracefully —
    the caller will bind the project on first request.
    """
    legacy_marker = ARTIFACT_BASE / "element_list.json"
    if not legacy_marker.exists():
        return
    # Determine the target project slug from the active project file.
    slug = None
    if ACTIVE_PROJECT_FILE.exists():
        try:
            slug = json.loads(ACTIVE_PROJECT_FILE.read_text(encoding="utf-8")).get("slug")
        except Exception:
            pass
    if not slug:
        # No active project yet — skip migration; it will be handled on first bind.
        return
...
def _init() -> None:
    """Deferred startup: migrate legacy layout and bind the active project.
    Called from main.py's create_app() instead of at module import time."""
    _migrate_legacy_layout()
    try:
        set_active_project(active_project())
    except ValueError:
        pass  # no active project yet; will be bound on first request


# _init() moved to main.py's create_app() — no module-level side effects.
