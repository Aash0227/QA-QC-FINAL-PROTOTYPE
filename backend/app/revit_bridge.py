...
READBACK_TOLERANCE_FT = float(os.environ.get("QAQC_READBACK_TOLERANCE_FT", "0.05"))
...
async def connection_locations_sequence(call: CallFn) -> dict[int, tuple[float, float]]:
    ...
    subset = re.search(r"(-\d{7,})", cat_res)
...
try:
    _env_cats = os.environ.get("QAQC_SCENE_CATEGORIES", "")
    if _env_cats.strip():
        SCENE_CATEGORIES = tuple(tuple(x) for x in json.loads(_env_cats))
    else:
        SCENE_CATEGORIES = _DEFAULT_SCENE_CATEGORIES
except (json.JSONDecodeError, ValueError):
    SCENE_CATEGORIES = _DEFAULT_SCENE_CATEGORIES
SCENE_MAX_PER_CATEGORY = int(os.environ.get("QAQC_SCENE_MAX_PER_CATEGORY", "3000"))
SCENE_BBOX_CHUNK = int(os.environ.get("QAQC_SCENE_BBOX_CHUNK", "300"))
