...
interface Ripple {
  x: number; y: number; radius: number; opacity: number; born: number;
}
...
export default function KineticGrid({
  ...
  const draw = useCallback((now: number) => {
    ...
    for (let row = 0; row < rows; row++) { pts[row] = []; prox[row] = [];
  ...
  // start/stop rAF when paused toggles
  useEffect(() => {
    if (paused) {
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
    } else {
  ...
  return (
    <div className={cn("relative w-full min-h-screen overflow-hidden bg-[#161618]", className)}>
      <canvas ref={canvasRef} className="fixed inset-0 w-full h-full z-0 pointer-events-none opacity-70" />
      <div className="relative z-10 w-full h-full">{children}</div>
    </div>
  );
}

/** Small toggle button for the parent to hide/show. Used in the island header. */
export function KineticGridToggle({
  paused,
  onToggle,
}: {
  paused: boolean;
  ...
  return (
    <button
      ...
      aria-label={paused ? "Play grid animation" : "Pause grid animation"}
    >
      {paused ? <Play className="h-3 w-3" /> : <Pause className="h-3 w-3" />}
      {paused ? "Grid paused" : "Grid"}
    </button>
  );
}